...
var tbody = document.getElementById('tabledb');

const BookManager = function(){
    this.printBooks = function(bookData){
        for(var i=0; i<bookData.length; i++){
            var tr = "<tr>";

            tbody.innerHTML += '<td><img src="'+bookData[i].cover+'"></td><td>' + bookData[i].title + '</td><td> ' + bookData[i].author + '</td><td> ' + bookData[i].price + '</td><td> ' + bookData[i].rating + '</td>';
        }
    }
    ...
    this.searchBooks = function(){
        var category = document.getElementById("query");
        var value = document.getElementById("value");
        if(category.selectedIndex === 0){
            var isbn = value.value;
            var listBooks = this.getBooksByISBN(isbn);
            this.printBooks(listBooks);
        }
        else if(category.selectedIndex === 1){
            var author = value.value;
            var listBooks = this.getBooksByAuthor(author);
            this.printBooks(listBooks);
        }
        else {
            var title = value.value;
            var listBooks = this.getBooksByTitle(title);
            this.printBooks(listBooks);
        }
